...
def workspace_delivery_thinking_rules(root: Path, project_id: str) -> list[str]:
    return [
        ...
        "",
        "downstreamAgent means one next receiver only. Put later planned roles such as Architecture -> Development -> Test in handoffChain; do not list multiple Agents in downstreamAgent.",
        "",
...
def workspace_start_delivery_thinking_rules(root: Path) -> list[str]:
    return [
        ...
        "",
        "downstreamAgent 只表示一个下一棒主要接收方。架构 -> 研发 -> 测试这类后续路线写入 handoffChain，不要把多个 Agent 塞进 downstreamAgent。",
        "",
