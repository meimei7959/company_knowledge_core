...
def workspace_delivery_thinking_rules(root: Path, project_id: str) -> list[str]:
    return [
        ...
        "",
        "Project Manager Agent must define an OutcomeSlice before formal decomposition, dispatch, handoff, acceptance routing, or risk escalation. An OutcomeSlice states the stage goal, primaryAgent, upstream/downstream Agents, risk-triggered escalation Agents, main deliverable, current state, target state, evidence, WIP/token/time guardrail, and stop conditions. Tasks are only a means to move that outcome state or reduce uncertainty.",
        "",
...
def workspace_start_delivery_thinking_rules(root: Path) -> list[str]:
    return [
        ...
        "",
        "项目经理 Agent 正式拆分、派发、交接、验收路由或风险升级前，必须先创建 OutcomeSlice：阶段目标、primaryAgent、上下游 Agent、风险触发时才介入的升级 Agent、主交付物、当前状态、目标状态、证据、WIP/token/time 护栏和停止条件。任务只是推动成果状态变化或降低关键不确定性的手段。",
        "",
