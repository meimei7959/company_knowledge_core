...
def workspace_delivery_thinking_rules(root: Path, project_id: str) -> list[str]:
    return [
        ...
        "",
        "Project Manager Agent must define an OutcomeSlice before formal decomposition, dispatch, handoff, acceptance routing, or risk escalation. An OutcomeSlice states the stage goal, primaryAgent, upstream Agent, single next downstreamAgent, later handoffChain, risk-triggered escalation Agents, main deliverable, current state, target state, evidence, WIP/token/time guardrail, and stop conditions. Tasks are only a means to move that outcome state or reduce uncertainty.",
        "",
        "downstreamAgent means one next receiver only. Put later planned roles in handoffChain as separate canonical Agent id list items; do not write one arrow string such as agent.company.architecture -> agent.company.development -> agent.company.test, and do not list multiple Agents in downstreamAgent.",
        "",
        "All OutcomeSlice Agent fields must use canonical Agent ids such as agent.company.product-manager, agent.company.design, agent.company.development, and agent.company.test. Do not use display names such as QA Agent, PM Agent, Admin Dev, or slash-separated role lists.",
        "",
        "After creating or editing local OutcomeSlice files, validate this workspace before continuing:",
        "",
        "```bash",
        f"python3 {root}/scripts/validate_workspace_outcome_slices.py --workspace .",
        "```",
        "",
        "One stage has one primary Agent. PM owns orchestration, evidence checks, blocker handling, and acceptance routing; the primary Agent owns the stage output. Other Agents join only when the OutcomeSlice escalation rule is triggered.",
        "",
        "When asked to reread rules, inspect progress, or make a phase plan, Project Manager Agent must proactively create or bind an OutcomeSlice after fact discovery. Do not wait for the user to ask for it. If facts are insufficient, record the smallest evidence-collection action that makes the OutcomeSlice decidable.",
        "",
        "Project Manager Agent owns cost control. Before any extra read, search, Agent dispatch, or analysis loop, it must state which OutcomeSlice state change or uncertainty reduction the spend will produce. If the spend cannot produce new evidence, a decision, risk reduction, or state transition, stop and write the stop reason.",
        "",
...
def workspace_start_delivery_thinking_rules(root: Path) -> list[str]:
    return [
        ...
        "",
        "项目经理 Agent 正式拆分、派发、交接、验收路由或风险升级前，必须先创建 OutcomeSlice：阶段目标、primaryAgent、上游 Agent、单一下一棒 downstreamAgent、后续 handoffChain、风险触发时才介入的升级 Agent、主交付物、当前状态、目标状态、证据、WIP/token/time 护栏和停止条件。任务只是推动成果状态变化或降低关键不确定性的手段。",
        "",
        "downstreamAgent 只表示一个下一棒主要接收方。架构、研发、测试这类后续路线写入结构化 handoffChain 列表，每项一个规范 Agent ID；不要写成 agent.company.architecture -> agent.company.development -> agent.company.test 这种箭头字符串，也不要把多个 Agent 塞进 downstreamAgent。",
        "",
        "OutcomeSlice 的所有 Agent 字段必须写规范 Agent ID，例如 agent.company.product-manager、agent.company.design、agent.company.development、agent.company.test。不要写 QA Agent、PM Agent、Admin Dev，也不要写斜杠分隔的多个角色。",
        "",
        "创建或修改本地 OutcomeSlice 文件后，继续任务前必须先校验当前项目工作区：",
        "",
        "```bash",
        f"python3 {root}/scripts/validate_workspace_outcome_slices.py --workspace .",
        "```",
        "",
        "一个阶段只有一个主责 Agent。PM 负责调度、证据检查、阻塞处理和验收路由；阶段产物由 primaryAgent 负责。其他 Agent 只有命中 OutcomeSlice 的升级规则时才介入。",
        "",
        "当用户要求重读规则、检查进展或制定阶段计划时，项目经理 Agent 在事实发现后必须主动创建或绑定 OutcomeSlice，不能等用户追问。如果事实不足，就记录最小补证动作，说明缺哪个事实才能让 OutcomeSlice 可判定。",
        "",
        "项目经理 Agent 负责成本控制。每次继续读文件、搜索、派 Agent 或追加分析前，必须判断这次消耗会推动哪个 OutcomeSlice 状态变化或降低哪个关键不确定性；如果不能产生新证据、决策、风险降低或状态变化，必须停损并说明原因。",
        "",
