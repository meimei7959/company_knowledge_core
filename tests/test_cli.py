...
""",
    ...
    def test_enterprise_agent_capability_control_plane_from_task_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("simulation", status)
            self.assertTrue(status["simulation"]["stable"])
            self.assertEqual("stabilize_candidate", status["capability"]["evaluation"][0]["recommendation"])
            self.assertEqual("draft", status["capability"]["releases"][0]["lifecycleState"])
            stdout = io.StringIO()
