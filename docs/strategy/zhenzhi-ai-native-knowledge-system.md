...
- 员工需要一个统一入口来发起项目、提交资料、查知识、申请工具和追踪状态。
- 公司会有很多台电脑、服务器和本地环境，它们是分布式执行资源，但现在没有统一注册、调度和结果回收。
- 经验留在个人脑子、聊天记录、本地笔记里，团队无法复用。
- 多个项目并行，但项目上下文、决策、进展、工具、经验没有统一沉淀。
- 未来项目开发会由数字员工、Agent 和分布式电脑上的 Codex、Claude、本地模型、工具完成，但现在缺少中央调度、Runner 注册、权限、复盘、知识写回机制。
...
```txt
先把项目入口、中央调度、Agent Ring 分布式执行、结果回填、知识沉淀流程跑起来，再逐步产品化。
```

## 2. 知识工程的定位

知识工程不是代码仓，不是万能执行器，也不是大而全的数据平台。
...
它负责决定任务如何流转、记录所有项目和知识状态，并和外部 Agent Ring 通信。Agent Ring 是装在各台电脑上的外部工作台/连接器，让每台电脑成为分布式处理器。

它在当前阶段承担 8 件事：

1. 管入口：飞书 Agent Hub 识别意图，发起项目、资料、任务、权限和查询流程。
2. 管项目上下文：每个项目做什么、为什么做、当前状态、关键决策。
3. 管任务调度：把资料处理、项目初始化、工程动作、知识沉淀变成可追踪任务卡，调度到具备能力和权限的 Agent Ring Runner。
4. 管 Agent：哪个 Agent 做什么、能访问什么、能调用什么工具、结果如何。
5. 管工具资产：谁开发了什么工具、在哪里、怎么用、什么风险、谁维护。
6. 管原始资料：会议纪要、文档、聊天、学习资料先登记为 SourceMaterial，保留来源、权限和证据链。
7. 管经验知识：做项目、调 Agent、写代码、交付客户过程中形成的可复用经验。
8. 管知识质量：所有落库内容先过知识审核 Agent，避免知识库变成低质量资料堆。

关键边界：

- Agent Hub 是入口、调度台和状态通知器，不是所有项目的中央工程执行器。
- 调度器和知识工程在本项目中生长，是中央处理器的一体两面。
- Agent Ring 不在本项目内实现；本项目只定义协议、注册模型、任务状态、审核和写回合同。
- 服务器小模型只做意图识别、字段抽取和流程路由，不承担长文档深度解析和工程交付。
- 工程类任务由被调度的 Agent Ring Runner 驱动本机 Codex、Claude、本地模型、浏览器或脚本完成。
- Runner 完成后必须把任务状态、结果摘要、执行 Agent、证据、SourceMaterial 和 KnowledgeItem 草稿写回知识工程。
...
| 项目需求 | 当前先用知识工程项目卡和任务卡；以后迁移到自研项目管理系统 | projectRef、任务摘要、状态、决策、约束 |
| 项目任务 | 中央调度器任务卡，未来迁移到项目管理系统 | taskId、能力要求、Runner、来源、状态、结果、通知记录 |
| Agent Runner | Agent Ring 注册的分布式电脑执行节点 | runnerId、能力、Agent、工具、权限、心跳、负载、租约 |
...
| 原始资料 | 云盘、文档、聊天、会议、客户资料、对象存储 | sourceRef、摘要、抽取事实、权限、引用链 |
| 学习资料 | 原网站、公众号、视频平台、飞书云空间、对象存储 | sourceRef、版权/许可、摘要、技能标签、适用场景、提炼后的 KnowledgeItem |

原则：

- Git 是代码真源，知识工程不保存完整代码。
- 项目管理系统上线前，知识工程临时承担项目、任务、Runner 注册和调度状态真源。
- 工具必须先登记，再允许团队复用或 Agent 调用。
- Agent 不能直接绕过知识工程访问工具和项目资料。
- 学习资料的原文、原视频、安装包、截图和导出文件不直接进入可复用知识；知识工程只保存引用、摘要、结构化提炼结果和使用边界。
...
- 团队知道每个项目当前在做什么。
- 调度器知道每个待处理事项需要什么能力、应该调度到哪台电脑、当前状态是什么、何时回填。
- 团队知道有哪些 Agent Ring Runner 在线、它们能调用什么 Agent 和工具。
- 团队知道有哪些工具可用、怎么用、谁维护。
- 团队知道有哪些 Agent、能做什么、有哪些权限。
- 飞书资料、会议纪要和项目输入能先形成 SourceMaterial，再生成任务并调度到合适 Runner。
- 项目经验能从分布式电脑和本地 Agent 工具沉淀到团队知识库。
- 好文章、视频、公众号、安装包等学习资料能被收集、解析、归类，并转成 Agent 可读的技能知识。
...
### 5.2 ProjectTask / KnowledgeTask

任务卡是项目调度和知识沉淀的核心中间对象。

每个需要中央调度器处理或派发给 Agent Ring 的事项必须有任务卡：
...
- `project_init`：项目初始化、仓库接入、项目群绑定、Agent 编组。
- `knowledge_capture`：会议纪要、文档、聊天、学习资料的结构化沉淀。
- `engineering_action`：需要匹配 Agent Ring Runner，并由 Runner 驱动本机 Codex、Claude、本地模型、IDE 或批准工具完成的工程动作。
- `tool_request`：工具或技能登记、风险分类、owner 审批。
- `handoff`：项目交接、资料同步、权限收口。

任务状态：

- `pending`：已创建，等待中央调度器匹配 Runner。
- `processing`：Agent Ring Runner 已领取任务并拉取上下文。
- `submitted`：已提交处理结果，等待 Review 或确认。
- `reviewing`：进入知识审核或 owner 审核。
- `done`：已完成并通知相关人。
- `blocked`：缺权限、缺资料或无法判断。
- `rejected`：不适合处理或不应入库。
...
AgentRunner 是外部 Agent Ring 注册进中央处理器的分布式电脑执行节点。

它不是本项目内实现的客户端，而是中央处理器看到的 Runner 记录。
...
- 哪些电脑在线。
- 哪些电脑能运行 Codex、Claude、本地模型、浏览器、Git 或特定脚本。
- 哪些电脑能访问哪些项目、仓库、资料和账号。
- 哪些电脑当前空闲，哪些任务正在运行。
- 调度器如何选择执行节点，如何失败重试。
...
- Agent Ring Runner 完成任务后有结构化回填。
- 机器人可以基于状态变化通知提交人。
- 知识审核可以追溯原始材料、处理过程和证据。
...
- decision：已做决策。
- lesson：项目经验。
- learning_note：学习资料提炼出的可复用笔记。
- skill_note：可迁移到 Agent 工作中的技能步骤、工具用法、排障方法。
- pattern：可复用做法。
- constraint：约束。
- issue：问题和坑。
- prompt：可复用提示词。
...
- 治理分类：
  ...
  - `auto_observed_learning`：低风险学习资料摘要、技能笔记、公开文章/视频提炼结果。机审通过后直接落库为 `observed/draft`，但必须保留 sourceRef、来源标题、作者/平台、许可/版权判断、适用场景和不确定点。
...
-> 创建 KnowledgeTask
-> 调度器匹配 Agent Ring Runner
-> Runner 驱动本机 Codex / Claude / 本地模型解析资料
-> 写回 TaskResult + AgentRun + KnowledgeItem 草稿 + 证据引用
-> 知识审核 Agent 审核草稿并治理分类
-> 通知提交人完成、补资料、冲突、拒绝或发起人工审批
```
...
-> 关联 ProjectTask / KnowledgeTask
-> 本地 Agent 根据任务上下文生成结构化草稿
-> 知识审核 Agent 审核草稿并治理分类
...
```txt
飞书机器人或 CLI 收到学习资料 URL / 公众号文章 / 视频 / 文档 / 图片 / 安装包
-> 保存原始资料引用或文件哈希，创建 SourceMaterial
-> 资料解析工具按类型抽取正文、转录、OCR、元数据或包信息
-> 知识提取 Agent 生成 learning_note / skill_note / pattern / issue 草稿
-> 知识审核 Agent 检查来源、版权/许可、摘要质量、分类、重复、冲突、敏感信息和可用性
-> 低风险学习笔记 observed/draft 落库，或补资料/冲突/拒绝/人工审批
```

学习资料的目标不是保存“看过什么”，而是沉淀“以后 Agent 做类似任务时应该怎么做、引用什么、避开什么坑”。
...
- 网页/公开文章：抓取标题、作者、平台、发布时间、URL、摘要和关键观点；不把全文复制进知识库。
- 公众号文章：优先保存原链接和转发文本；链接不可读时，允许用用户转发正文、截图 OCR 或 PDF 导出作为 SourceMaterial，并标明抽取方式和置信度。
- 视频/音频：先生成 transcriptRef、章节摘要、关键步骤、工具/术语清单，再提炼 KnowledgeItem。
- PDF/Office/图片：先做文本抽取或 OCR，记录页码/区域来源，再提炼结构化知识。
- 安装包/二进制/模型文件：不进入 RAG 正文；只保存 storageRef、contentHash、版本、来源、许可、用途、风险、安装/校验说明和负责人。
- 代码仓资料：只抽 README、API 文档、CHANGELOG、提交摘要和经验复盘，不复制完整源码。

### 5.9 NotificationRecord

通知记录保存机器人对提交人、负责人、项目群的关键通知。
...
NotificationRecord 解决：

- 知道提交人是否已经收到“已接收/已完成/需补资料”。
- 知道 Runner 或治理 owner 是否已经收到任务卡。
- 通知失败时可以重试或人工处理。
...
| Project | 项目卡 |
| ProjectTask | 项目任务卡 |
| KnowledgeTask | 知识沉淀任务卡 |
| AgentRunner | 外部 Agent Ring 注册的分布式执行节点 |
| TaskResult | 任务处理结果 |
| NotificationRecord | 机器人通知记录 |
| Agent | Agent 身份和权限 |
| ToolAsset | 工具资产 |
...
知识图谱不是第一天就上复杂图数据库，但关系必须从第一天记录、校验、索引和导出。

这里的图谱不是独立真源，而是知识工程管理层。它把 Project、Task、SourceMaterial、TaskResult、KnowledgeItem、AgentRun、ToolAsset、Decision、Policy、AuditLog 之间的关系变成可查询边，用于影响分析、证据链、stale 检测、权限解释、Agent context pack 生成和可视化导航。
...
先用 Markdown/YAML 字段和 PostgreSQL 关系表记录关系，后续再按需要进入图数据库。不要再引入 SQLite 作为本地或线上运行路径。

第一阶段可物化为 `KnowledgeGraphEdge`：

```txt
fromRef + relation + toRef + sourceRef/evidenceRefs + confidence + status + sensitivity + auditRefs
```

后续可导出 `GraphSnapshot` 给可视化、影响评估或 API 同步。GraphSnapshot 是导出物，不是真源；真源仍是经过审核的对象文件和 AuditLog。
...
- 一次代码变更产生了哪些经验。
- 一个客户交付物引用了哪些事实。
- 一个 Decision 或 Policy 变更会影响哪些任务、知识和 Agent。
- 一个 context pack 为什么包含或排除了某条知识。
- 一个 stale candidate 应该沿哪些关系传播。

治理要求：

- 图谱边继承两端对象和证据的最高敏感级别。
- secret 不允许进入图谱边、图谱快照、索引或审计正文。
- customer_confidential 图谱视图默认只在项目/客户范围内可见。
- 自动抽取的边只能作为 observed/draft 管理信息；涉及 verified、approved、active、权限、安全、客户承诺、跨团队标准时仍需对应人工审核。
...
新项目开始时，Agent Hub 或项目治理 owner 注册项目；中央调度器创建初始化任务并匹配 Agent Ring Runner。

项目初始化必须闭环。登记项目只是第一步，默认由项目级 `agent.<project>.project-manager` 接管 `project_initialization` 任务。它负责确认 `launch.md`、项目 Owner、仓库模式、项目群、Agent team、Runner 或人工接管路径、Review 规则、风险和首批任务。

创建项目动作只能生成启动里程碑：M0 信息收齐或缺口明确，M1 Owner 和审批路径明确，M2 仓库/项目群/默认 Agent team/Runner/Review 规则准备好或阻塞原因明确，M3 首批可执行 ProjectTask 已创建。业务里程碑、产品交付日期、研发 Sprint 不能由创建项目动作猜测，必须由项目经理 Agent 基于上下文提出，并由人类项目 Owner 确认。

默认 Agent team 是项目经理 Agent、产品经理 Agent、架构师 Agent、研发 Agent、测试 Agent、知识工程 Agent；只有当 intake 明确说明需求已拆清、仅知识/运营/简单配置迁移或不需要产品时，才跳过产品经理 Agent，并在 `launch.md` 记录原因。涉及系统边界、接口、数据模型、安全、性能、可靠性、长期演进或高风险代码时，必须由架构师 Agent 输出架构/技术方案，研发 Agent 再负责实现。产品需求澄清不能交给通用项目经理 Agent；需要产品澄清时由产品经理 Agent 或明确的人类产品 Owner 承接。用户输入的前端、后端、测试、运营或领域角色先进入候选清单，再由项目经理 Agent 根据 Runner、权限、审批和实际任务转成项目 Agent 或首批 ProjectTask。项目只有在写回 `TaskResult` 与 `AgentRun` 或人工接管记录，并通知请求人/项目 Owner 后，才算完成初始化。
...
- [ ] 迁移为 PostgreSQL 索引；本地开发和线上使用同一 PostgreSQL schema。
- [x] 支持按 projectId 查询。
- [x] 支持按 agentId 查询。
- [x] 支持按 toolId 查询。
- [x] 支持按 status 查询。
...
### 15.9.1 P1：学习资料和原始资料解析

- [ ] 实现 `material ingest`，把 URL、公众号文章、视频、音频、图片、文档、安装包、二进制、模型文件、数据集登记为 SourceMaterial。
- [ ] 实现 `material extract`，按 materialType 调用网页/文章解析、公众号兜底解析、转录、OCR、文档解析、包登记等工具。
- [ ] SourceMaterial 写入 materialType、sourceRef、storageRef、contentHash、license、sensitivity、extractionTool、extractionStatus。
- [ ] 网页和公众号文章只入库摘要、关键观点、技能步骤和引用链接，不把全文作为 KnowledgeItem。
- [ ] 视频/音频必须先形成 transcriptRef 或章节摘要，再提炼 learning_note / skill_note。
- [ ] 图片/截图必须记录 OCR 或视觉摘要方式和置信度。
- [ ] 安装包、二进制、模型文件、数据集只登记哈希、版本、来源、许可、用途、风险、负责人和安装/校验说明，默认不进入 RAG 正文。
- [ ] 知识提取 Agent 能把学习资料转成 learning_note、skill_note、pattern、issue 草稿。
- [ ] 知识审核 Agent 能判断低风险学习笔记直接 observed/draft，来源/许可/敏感/高风险不清时转补资料或人工审批。
- [ ] 飞书机器人支持“学习资料 / 公众号文章 / 视频资料 / 安装包”四类短指令，并返回简短状态。

验收标准：

- 同事丢一篇文章、一个视频或一个安装包给机器人时，系统能保存原始引用并生成 Agent 可读的结构化草稿。
- 原始全文、完整转录、安装包和二进制不会直接进入可复用知识或 RAG。
- 每条学习资料衍生知识都能追溯 sourceRef、提取工具、许可/敏感判断和适用场景。

### 15.9.2 P1：知识图谱管理

- [ ] 定义 `KnowledgeGraphEdge` 对象，字段包含 fromRef、relation、toRef、sourceRef/evidenceRefs、confidence、status、sensitivity、auditRefs。
- [ ] 定义 `GraphSnapshot` 导出物，明确它只用于可视化、影响分析、API 同步或离线审计，不作为真源。
- [ ] 从 Project、ProjectTask、KnowledgeTask、AgentRunner、TaskResult、SourceMaterial、KnowledgeItem、AgentRun、ToolAsset、Decision、Policy、ConflictRecord、EvalRun、ReviewRecord、AuditLog 中抽取基础边。
- [ ] 在 `rebuild_index` 或等效索引流程中物化图谱边。
- [ ] 实现 `zhenzhi-knowledge graph export`，输出 nodes/edges JSON，默认过滤 secret，customer_confidential 只允许项目/客户范围导出。
- [ ] 实现 `zhenzhi-knowledge graph impact <ref>`，用于展示工具、项目、知识、source material、Decision、Policy、代码引用变化的影响范围。
- [ ] `start` 生成 context pack 时说明知识纳入、排除、审批门槛的关系原因。
- [ ] Knowledge Engineering Agent review sub-agent gate 检查新增知识或状态变更的图谱影响、重复风险、冲突风险和 stale 传播。
- [ ] stale 检测沿 ToolAsset、SourceMaterial、Project、Decision、Policy、codeRefs 等边生成 stale_candidate。
- [ ] 图谱边和快照写入、删除、导出必须有 AuditLog。

验收标准：

- 能从一个 ToolAsset、SourceMaterial、KnowledgeItem、Decision 或 Policy 反查受影响项目、任务、知识、AgentRun 和审核事项。
- 能在不引入复杂图数据库的情况下导出可视化图谱 JSON。
- 图谱不会绕过权限、审核、敏感级别和 secret 禁止规则。
- GraphSnapshot 可删除或重建，删除后不影响真源对象。
...
Graph Steward 负责关系字段、KnowledgeGraphEdge 抽取规则、GraphSnapshot 导出规则、影响分析口径和图谱敏感级别传播。Graph Steward 不审批 verified 知识或 approved 工具，只保证图谱关系可解释、可审计、可重建，并把冲突、stale、权限异常交给对应 owner/reviewer。

#### 审核标准

verified KnowledgeItem 必须满足：有明确来源、可复查、非 secret、非未授权客户敏感正文、scope 合理、confidence 明确、没有被更新决策或 stale candidate 覆盖。

涉及 verified、approved、active、权限、安全、客户承诺或跨团队标准的状态变更，必须先查看图谱影响范围。影响对象不清楚时，不能直接发布，只能保持 draft/testing/stale_candidate 或进入补资料流程。
...
ToolAsset 默认策略由 riskLevel 生成：L1-L2 为 agent_policy_allowed；L3-L5 为 approval_required。工具调用、结果写入项目、结果进入知识库、状态提升必须分开治理。低风险已注册工具可被更广泛地发现和 dry-run；L3-L5 或有外部副作用的真实执行必须经过明确审批。工具结果默认只返回给请求者，不自动成为项目资料或可复用知识。写入项目需要项目成员或 owner 权限；进入知识库必须先成为结构化 draft 并通过 Knowledge Engineering Agent review sub-agent gate；提升为 verified、approved、active、跨团队标准或高风险结果必须由相应 human owner 审批。
